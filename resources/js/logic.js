...
function OnNumber(value) {
  operatorActive = 0;
  if(document.getElementById('mainDisplay').innerHTML.trim()=="0" || scrReset==1) {
    scrReset=0;
...
function OnOperator(value) {
  ...
  if (document.getElementById('mainDisplay').innerHTML.slice(-1) == ".") {
    document.getElementById('mainDisplay').innerHTML = document.getElementById('mainDisplay').innerHTML.slice(0, -1);
  }
  if (operatorActive == 1) {
    document.getElementById('secDisplay').innerHTML = (document.getElementById('secDisplay').innerHTML.slice(0,-2) + value + " ");
  }else{
  document.getElementById('secDisplay').innerHTML += document.getElementById('mainDisplay').innerHTML
  document.getElementById('mainDisplay').innerHTML = eval(document.getElementById('secDisplay').innerHTML);
...
function OnNegate() {
  ...
  if(neg==1){
    document.getElementById('mainDisplay').innerHTML = "-" + document.getElementById('mainDisplay').innerHTML;
  } else if (neg==-1) {
    document.getElementById('mainDisplay').innerHTML = eval("-(" + document.getElementById('mainDisplay').innerHTML + ")")
  }
}
...
function shwHideHistory() {
  if (shwLogActive==0) {
    ...
    document.getElementById("History").style.animation = "slideUp 0.2s ease-out 0s normal forwards";
    shwLogActive=1;
  }else {
